...
import com.apajac.acolhimento.controllers.usuario.BuscaUsuariosPorNomeController;
...
class BuscaUsuariosPorNomeControllerTest {
    ...
    @InjectMocks
    private BuscaUsuariosPorNomeController buscaUsuariosPorNomeController;

    @Test
    void dadoNomeValido_quandoBuscarUsuariosPorNome_entaoRetorneListaUsuarioDTO() {
        ...
        Mockito.when(usuarioService.buscarUsuariosPorNome(nome, pageable)).thenReturn(usuarioPage);
        Mockito.when(usuarioMapper.convertEntitesToDtos(usuarioEntities)).thenReturn(usuarioDTOs);

        // Chama o método a ser testado
        ResponseEntity<ListaUsuarioDTO> response = buscaUsuariosPorNomeController.buscarUsuariosPorNome(nome, pageable);
    ...
    @Test
    void dadoExcecao_quandoBuscarUsuariosPorNome_entaoLancaHttpClientErrorException() {
        // Configuração dos parâmetros de entrada
        String nome = "Usuario";
        Pageable pageable = PageRequest.of(0, 10);

        // Configuração do comportamento esperado do serviço para lançar uma exceção
        HttpClientErrorException exception = new HttpClientErrorException(HttpStatus.BAD_REQUEST, "Erro ao buscar usuários");
        Mockito.when(usuarioService.buscarUsuariosPorNome(nome, pageable)).thenThrow(exception);

        // Verifica se o método lança a exceção esperada
        HttpClientErrorException thrown = assertThrows(HttpClientErrorException.class, () -> {
            buscaUsuariosPorNomeController.buscarUsuariosPorNome(nome, pageable);
        });

        // Verifica o status e a mensagem da exceção
        assertEquals(HttpStatus.BAD_REQUEST, thrown.getStatusCode());
        assertEquals("400 400 Erro ao buscar usuários", thrown.getMessage());
    }
}
